...
function getTrades(ticker, from, to) {
  ...
  const values = [
    ["Date", "Operation", "Payment",  "Commission", "Price", "Quantity"], 
  ]
  for (let i=0; i<=operations.length-1; i++) {
    const {operationType, status, trades, date, commission} = operations[i]
    ...
    values.push([
      isoToDate(date), operationType, totalSum, -commission.value, weigthedPrice, totalQuantity
    ])
  }
  return values
}

function getTrades2(ticker, from, to) {
  const figi = _getFigiByTicker(ticker)
  if (!from) {
    from = TRADING_START_AT.toISOString()
  }
  ...
  for (let i=0; i<=operations.length-1; i++) {
    ...
    values.push([
      ...
      values.push([
      isoToDate(date), operationType, totalSum, 0, weigthedPrice, totalQuantity
    ])
    }
  }
  return values
